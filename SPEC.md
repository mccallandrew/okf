...
§13. Worked examples are in Appendix A (attested computations) and
Appendix B (a domain rules wiki).
...
## Appendix B: Worked example, a domain rules wiki

Appendix A shows OKF on metrics and attested computations. This appendix shows the same format used as a **domain knowledge wiki**: a large concept graph with provenance to an external authority, no Attested Computation concepts required.

The repository ships a full example at [`bundles/mtg-rules/`](bundles/mtg-rules/): Magic: The Gathering rules organized by idea (foundations, zones, turn, stack, card types, combat, keywords, effects, multiplayer, variants), with Comprehensive Rules citations in each concept's `sources` frontmatter.

```
bundles/mtg-rules/
  index.md                         okf_version: "0.2"
  log.md
  foundations/                     Game concepts (golden rules, SBAs, mana, …)
  zones/                           Library, hand, battlefield, stack, …
  turn/                            Phases, steps, priority, special actions
  stack-and-priority/              Casting, modes, triggers, targets
  card-types/                      Types, subtypes, DFC, Adventure
  combat/                          Attack, block, combat damage
  keywords/                        Evergreen abilities + keywords/actions/
  effects/                         One-shot, continuous, layers, replacement
  multiplayer/                     APNAP, THG, Commander overview
  variants/                        Planechase, Archenemy, Limited
  references/comprehensive-rules.md
```
...
title: Flying
description: Evasion — can only be blocked by creatures with flying or reach.
tags: [mtg, keywords, evergreen, flying]
status: stable
...
sources:
  ...
  - id: cr-702-9
    resource: /references/comprehensive-rules.md
    title: "CR 702.9 Flying"
---

# Definition

**Flying** is an evasion ability. A creature with flying can be blocked only by
creatures with flying or [reach](/keywords/reach.md).[^cr-702-9]

[^cr-702-9]: CR 702.9 Flying
```

What this example exercises from the format:

- Bundle structure, reserved `index.md` / `log.md`, and root `okf_version` (§3, §8, §9, §12).
- Producer-defined `type` values (`Keyword Ability`, `Zone`, `Rule Concept`, …) with recommended `title`, `description`, `tags`, and `status` (§4, §5.4).
- Provenance via `sources` pointing at an in-bundle Reference that itself carries an external `resource` URL (§5.1, §6.3).
- Cross-links as the relationship graph; organization by topic rather than by the source document's numbering (§6).
- No attestation family — appropriate when the corpus explains rules rather than computing values (§10).
