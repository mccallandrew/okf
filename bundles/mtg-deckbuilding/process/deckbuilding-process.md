...
2. **Gather candidates** — Role piles: threats, answers, mana, card selection (use [Scryfall](/builder-resources/scryfall.md) / format tools). Optional: write an [8-by-8](/references/eight-by-eight-edh.md) recipe or name ~[eight packages](/commander/packages.md) before shopping for pets.[^eight-by-eight][^341-build]
...
* **Commander**: Vision → commander (lock or discover) → packages → mana → tune against the [local meta](/commander/local-meta.md).[^341-build] Consult [EDHREC](/builder-resources/edhrec.md) after the plan is set, not before—creativity first, then tools.[^341-build]
